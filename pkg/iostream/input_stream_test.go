...
func TestReaderImpl(t *testing.T) {
	r := NewInputStream(bytes.NewBuffer(nil))
	_, err := r.Read([]byte{})
	assert.Error(t, err)
}

func TestNewReader(t *testing.T) {
	r1 := NewInputStream(bytes.NewBuffer(nil))
	r2 := NewInputStream(r1)
	assert.Equal(t, r1, r2)
}

// assertRead asserts a single read operation
func assertRead(t *testing.T, name string, fn func(*InputStream) (interface{}, error), input []byte, expect interface{}) {
	assertReadN(t, name, fn, input, expect, 99999)
}

// assertReadN asserts a single read operation
func assertReadN(t *testing.T, name string, fn func(*InputStream) (interface{}, error), input []byte, expect interface{}, max int) {
	if max > len(input) {
		max = len(input)
	}
	...
	t.Run(name, func(t *testing.T) {
		...
		src := newFakeSource(input[:max])
		rdr := NewInputStream(src)
		out, err := fn(rdr)
...
func assertSliceRead(t *testing.T, name string, fn func(*InputStream) (interface{}, error), input []byte, expect interface{}) {
	t.Run(name, func(t *testing.T) {
		msg := fmt.Sprintf("write %v", name)
